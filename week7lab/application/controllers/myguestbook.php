...
class MyGuestBook extends CI_Controller {
  ...
  function view() {
    $data['result'] = $this->myguestbook_model->read(null, null, null);
      ...
      // Check if $data['result'] is null
    if ($data['result'] === null) {
        // If no records found, redirect to the same page
      $this->load->view('mainmenu', $data);
    ...
    // Check if the record exists
      $existingRecord = $this->myguestbook_model->read(array('id' => $id), null, null);

      if (!$existingRecord) {
        // If the record does not exist, redirect to the list of all comments
        $this->view();
        return; // Make sure to stop further execution
      }
